...
\needs bounds        : bounds        OVER + SWAP ;
[IFNDEF] round-up
 : round-up  ( adr granularity -- adr' )  1-  tuck +  swap invert and  ;
[THEN]

\needs ftell	: ftell  ( fd -- byte# )  FILE-POSITION THROW drop  ;

: $read-file  ( filename$ -- adr len )
   $read-open
   MIFD @ fsize              ( len )
   dup alloc-mem swap       ( adr len )
   2dup MIFD @ fgets         ( adr len actual )
   over <> if               ( adr len )
      MIFD @ fclose          ( adr len )
   ...
   MIFD @ fclose             ( adr len )
;
: $add-file  ( filename$ -- )  $read-file  2dup  MOFD @ fputs  free-mem  ;

: $copy  ( src-filename$ dst-filename$ -- )
   $new-file  $add-file  MOFD @ fclose
;
...
: $file-size  ( filename$ -- len )
   ...
   MIFD @ fclose             ( len )
;
: putlong  ( n -- )  lbsplit  4 0 do  MOFD @ fputc  loop  ;
: write-dropin'  ( adr len expanded-len name-str -- )
   2>r >r                                          ( adr len )
   S" OBMD" MOFD @ fputs                             ( adr len )
   dup putlong                                     ( adr len )
   2dup 0 -rot bounds  ?do  i c@ +  loop  putlong  ( adr len )
   r> putlong                                      ( adr len )
   2r>  d# 16 min  tuck  MOFD @ fputs               ( adr len name-len )
   d# 16 swap  ?do  0 MOFD @ fputc  loop            ( adr len )
   tuck  MOFD @ fputs                               ( len )

   \ Pad out to a 4-byte boundary
   dup 4 round-up swap  ?do  1 MOFD @ fputc  loop   ( )
;
: write-dropin  ( adr len expanded-len name-str -- )
\ F7_ED
   ...
   MOFD @ ftell  over  4 round-up +  h# 20 +   ( adr len pos )
   ...
   reserved-start  reserved-end  within  if
      ...
      reserved-end MOFD @ ftell -  h# 20 -     ( adr len reslen )
...
: write-bth-header  ( name$ -- )
   ...
   " .bth" bth-name $new-file  MOFD @ to bth
